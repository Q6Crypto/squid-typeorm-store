...
describe('Store', function () {
    describe('.save()', function () {
        ...
        it('get single entity', async function () {
            ...
            await (0, expect_1.default)(store.get(model_1.Item, '1')).resolves.toEqual({ id: '1', name: 'a' });
        });
        it('get single entity with relation', async function () {
            let store = await createStore();
            const item = new model_1.Item('1', 'a');
            await store.save(item);
            await store.save(new model_1.Order({ id: '1', qty: 1, item }));
            await (0, expect_1.default)(store.get(model_1.Order, { id: '1', relations: { item: true } })).resolves.toEqual({
                id: '1',
                qty: 1,
                item: { id: '1', name: 'a' },
            });
        ...
        it('save single entity', async function () {
            let store = await createStore();
            await store.save(new model_1.Item('1', 'a'));
            await (0, expect_1.default)(getItems(store)).resolves.toEqual([{ id: '1', name: 'a' }]);
        });
        it('save multiple entities', async function () {
            let store = await createStore();
            await store.save([new model_1.Item('1', 'a'), new model_1.Item('2', 'b')]);
            await (0, expect_1.default)(getItems(store)).resolves.toEqual([
                { id: '1', name: 'a' },
                { id: '2', name: 'b' },
            ]);
        });
        ...
        it('updates', async function () {
            let store = await createStore();
            await store.save(new model_1.Item('1', 'a'));
            await store.save([new model_1.Item('1', 'foo'), new model_1.Item('2', 'b')]);
            await (0, expect_1.default)(getItems(store)).resolves.toEqual([
                { id: '1', name: 'foo' },
                { id: '2', name: 'b' },
            ]);
    ...
    describe('.remove()', function () {
        ...
        it('remove by passing an entity', async function () {
            ...
            await (0, expect_1.default)(getItemIds(store)).resolves.toEqual(['2', '3']);
        });
        it('remove by passing an array of entities', async function () {
            let store = await createStore();
            await store.remove([new model_1.Item('1'), new model_1.Item('3')]);
            await (0, expect_1.default)(getItemIds(store)).resolves.toEqual(['2']);
        });
        ...
        it('remove by passing an array of ids', async function () {
            ...
            await (0, expect_1.default)(getItemIds(store)).resolves.toEqual(['3']);
        });
    });
...
async function createStore() {
    const em = await (0, util_1.getEntityManager)();
    return new store_1.StoreWithCache(() => em, { commitOrder: (0, relationGraph_1.getCommitOrder)(em.connection.entityMetadatas) });
}
async function getItems(store) {
    return store.find(model_1.Item, { where: {} });
}
function getItemIds(store) {
