...
export class StoreWithCache extends Store {
  ...
  async remove<E extends EntityType>(
    entityClass: EntityTarget<E>,
    id: string | string[]
  ): Promise<void>;
  async remove<E extends EntityType>(
    e: E | E[] | EntityTarget<E>,
    id?: string | string[]
  ): Promise<void> {
    await this.currentCommit.waitForUnlock();
  ...
  async count<E extends EntityType>(
    entityClass: EntityTarget<E>,
  ...
  async find<E extends EntityType>(
    entityClass: EntityTarget<E>,
    options: FindManyOptions<E>
  ): Promise<E[]> {
    await this.commit();
  ...
  async findOneOrFail<E extends EntityType>(
    entityClass: EntityTarget<E>,
    options: FindOneOptions<E>
  ): Promise<E> {
    await this.commit();
  ...
  async findOneByOrFail<E extends EntityType>(
    ...
    const res = await super.findOneByOrFail(
      entityClass as EntityClass<E>,
      where
    );
  ...
  async getOrFail<E extends EntityType>(
    ...
    if (e == null) {
      const metadata = this.getEntityMetadata(entityClass);
      throw new Error(
        `Missing entity ${metadata.name} with id "${options.id}"`
      );
    }
  ...
  defer<E extends EntityType>(
    ...
    return new DeferredEntity({
      ...
      getOrInsert: async (create) =>
        this.getOrInsert(entityClass, options, create),
    });
  }
    ...
    await this.currentCommit.runExclusive(async () => {
      ...
      for (const { metadata, extraUpserts } of changeSets) {
        if (extraUpserts.length > 0) {
          log.debug(
            `commit extra upserts for ${metadata.name} (${extraUpserts.length})`
          );
    ...
    for (const [id, type] of changes) {
      ...
      switch (type) {
        case ChangeType.Insert: {
          assert(
            cached?.value != null,
            `unable to insert entity ${metadata.name} ${id}`
          );
        ...
        case ChangeType.Upsert: {
          assert(
            cached?.value != null,
            `unable to upsert entity ${metadata.name} ${id}`
          );
    ...
    await this.currentLoad.runExclusive(async () => {
      ...
      for (const [metadata, data] of defers) {
        ...
        for (let batch of splitIntoBatches(ids, 30000)) {
          ...
          await this.find<any>(metadata.target, {
            where: { id: In(batch) },
  ...
  private cloneEntity<E extends EntityType>(
    entity: E,
    mask?: FindOptionsRelations<any>
  ): E | undefined {
    const metadata = this.getEntityMetadata(entity.constructor);
      ...
      for (const relation of metadata.relations) {
        ...
        if (relation.isOneToMany || relation.isManyToMany) {
          ...
          for (const entity of inverseEntity) {
            this.traverseEntity(
              entity,
              inverseMask === true ? null : inverseMask,
              fn
            );
          }
        } else {
